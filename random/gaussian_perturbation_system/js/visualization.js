...
class MultiViewVisualization {
    ...
    renderPerturbedWithData(data) {
        ...
        // 绘制覆盖层 (Overlay)
        const view = this.views.perturbed;
        if (this.generator) {
            if (view.options.showGridLines) view.drawQuadrantGrid();
            if (view.options.showGaussianCenters) view.drawGaussianMarkers(this.generator, false);
            if (view.options.showGaussianBorders) view.drawGaussianBorders(this.generator, false);
        }

        // 更新扰动数据
        this.perturbedData = normalizedData;

        // 如果是软归因，更新计数显示为全部高斯（因为软归因是像素级的，很难界定"哪些高斯被扰动了"）
        // 或者保持原样，显示 param-perturbed Gaussians
        const perturbedCount = this.generator.getAllGaussians().filter(g => g.isPerturbed).length;
        document.getElementById('info-perturbed').textContent =
    ...
    /**
     * 更新所有视图
     * @param {Float32Array} softPerturbedData - 可选的软扰动数据
     */
    updateAllViews(softPerturbedData = null) {
        this.renderOriginal();

        const hasPerturbation = this.generator.getAllGaussians().some(g => g.isPerturbed);

        if (hasPerturbation) {
            if (softPerturbedData) {
                this.renderPerturbedWithData(softPerturbedData);
            } else {
